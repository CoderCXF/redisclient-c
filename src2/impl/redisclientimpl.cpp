...
namespace
{
    ...
    inline void bufferAppend(std::vector<char> &vec, char c)
    {
        ...
        // resize会实际的分配内存并进行初始化，而reserve只是预先保存，改变的是capacity的大小
        ...
        boost::system::error_code &ec)
{
    for(;;)
    {
        // 如果buffer里面没有数据了，我们就重新读取至buffer中
        if (bufSize == 0)
        {
            // 从服务器读取数据
        ...
        for(size_t pos = 0; pos < bufSize;)
        {
            ...
            // 如果结果是complete的话，就返回结果RedisValue
            if( result.second == RedisParser::Completed )
            {
                return redisParser.result();
            }
            // 否侧的话，没有incomplete，就继续读取，然后从buf中取数据
...
// 就是向服务器发送subscribe命令，以及订阅的通道名channel
    ...
    std::function<void(RedisValue)> handler)
{
    ...
    {
        std::deque<RedisBuffer> items{ command, channel };  // 创建命令队列
